...
export function TourItinerary({ tour }: TourItineraryProps) {
  ...
  const days = rawList<ItineraryDay>(t, `tours.${tour.slug}.days`);

  const items: AccordionItem[] = days.map((day, index) => {
    const image = tour.dayImages[index] ?? tour.dayImages[tour.dayImages.length - 1];
      ...
      header: (
        <span className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
          <span className="rounded-full bg-[#6a9d17] px-[11px] py-[4px] font-display text-[10px] font-black uppercase leading-none tracking-[0.04em] text-white">
            {t("common.dayNumber", { number: index + 1 })}
          </span>
