...
| Новые переводы в `seed.json` (например, немецкий и испанский) | `manage.py migrate` создаёт поля, а `manage.py seed_demo --translations` заполняет только пустые — существующие тексты не перезаписываются. Затем пересоберите сайт: список языков зашит в код |
