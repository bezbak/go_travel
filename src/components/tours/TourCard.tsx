...
export function TourCard({ tour, className, layout = "grid" }: TourCardProps) {
  ...
  return (
    ...
    >
      <div className="relative h-[210px] shrink-0 overflow-hidden bg-[#e7e2d9] 2xl:h-[236px]">
        <Image
          fill
          alt={t(tour.cardImage.altKey)}
          className="object-cover transition duration-500 group-hover:scale-[1.05]"
          sizes="(min-width: 1280px) 420px, (min-width: 640px) 46vw, 90vw"
          src={tour.cardImage.src}
        />
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-[14px]">
      ...
      <div className="flex flex-1 flex-col p-[20px] 2xl:p-[24px]">
        ...
        <h3 className="mt-[10px] font-display text-[19px] font-black uppercase leading-[1.1] tracking-[0] text-[#171717] 2xl:text-[21px]">
          <Link
        ...
        <dl className="mt-[16px] flex flex-wrap items-center gap-x-[18px] gap-y-2 text-[12px] font-semibold text-[#4f4f4f]">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 border-t border-[#eeebe3] pt-[16px] 2xl:pt-[18px]">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.04em] text-[#8a8a8a]">
              {t("common.from")}
            </p>
            <p className="flex items-baseline gap-2">
              <span className="font-display text-[22px] font-black leading-none text-[#171717]">
                {formatPrice(tour.priceEur, locale)}
              </span>
              {tour.oldPriceEur ? (
                <span className="text-[13px] font-semibold text-[#a0a0a0] line-through">
                  {formatPrice(tour.oldPriceEur, locale)}
                </span>
              ) : null}
            </p>
            <p className="mt-0.5 text-[11px] font-medium text-[#8a8a8a]">
              {t("common.perPerson")}
            </p>
          </div>
          ...
          <span
            aria-hidden="true"
            className="font-display text-[12px] font-extrabold uppercase text-[#669a17]"
          >
            {t("common.viewTour")}
          </span>
        </div>
